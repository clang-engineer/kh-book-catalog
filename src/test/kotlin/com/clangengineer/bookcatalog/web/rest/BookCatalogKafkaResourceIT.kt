...
import com.clangengineer.bookcatalog.config.EmbeddedKafka
import com.clangengineer.bookcatalog.config.KafkaSseConsumer
import com.clangengineer.bookcatalog.config.KafkaSseProducer
...
class BookCatalogKafkaResourceIT {
    ...
    fun producesMessages() {
        client.post().uri("/api/book-catalog-kafka/publish?message=value-produce")
            .exchange()
            .expectStatus()
            .isNoContent
    ...
    fun consumesMessages() {
        ...
        val value = client
            .get()
            .uri("/api/book-catalog-kafka/consume")
